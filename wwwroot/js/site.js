    ...
    $(document).on('click', '.sub-link', function (e) {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (cssHref) {
                    loadPageStylesheet(cssHref, showContentAndScripts);
                } else {
                    showContentAndScripts();
                }

                function showContentAndScripts() {
                    // Inject content
...
// ==============================
// Handle Back/Forward/Refresh navigation
// ==============================
window.addEventListener("popstate", function (event) {
    const state = event.state;
    if (state && state.url) {
        // Reload content via AJAX when navigating history
        $.ajax({
            url: state.url,
            type: 'GET',
            success: function (result) {
